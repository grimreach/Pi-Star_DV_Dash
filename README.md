...
| WiFi connect/disconnect (write) | `server/src/routes/wifi.ts` | Still mock, deliberately — changing what network the device is on is a separate, riskier decision than a read-only scan. UI shows scan results without connect/disconnect controls for now. |
| SSH status (read) | `server/src/pistar/sshStatus.ts` | **Real** — `systemctl is-enabled ssh`, no sudo needed for a read. |
| SSH toggle (write) | `server/src/routes/ssh.ts` | Still mock — actually changing it needs `sudo systemctl enable/disable ssh`, a new scoped grant like the config writer's. UI shows read-only status rather than a toggle that silently wouldn't do anything, until that's wired up. |
